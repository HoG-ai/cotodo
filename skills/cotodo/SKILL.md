...
4. Each iteration, first run `grep -n 'over$\|@delete' TODO.md` to locate all pending lines; don't only read file tail
5. Each iteration check `PAUSE:` — if `true`, immediately askQuestions to pause, do nothing else
6. Prioritize topics with `over` marker; when multiple, process top-to-bottom one by one
7. When starting to process, change `over` at end of user message to `[pending...]` (so user sees it's being processed); remove `[pending...]` after reply is complete
...
11. When `##` heading has `@delete` marker, delete the entire topic section (from `##` to before next `##`)
...
> User messages start with `User:`, end with `over` when done. Agent changes it to `[pending...]` while processing.
