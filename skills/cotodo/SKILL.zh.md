...
4. 每次循环先用 `grep -n 'over$\|@delete' TODO.md` 定位所有待处理行；不要只读文件尾部
5. 每次循环检查 `PAUSE:`，为 `true` 时立即 askQuestions 暂停，不做其他操作
6. 优先处理有 `over` 标记的话题，多个时从上到下逐个处理
7. 开始处理时将用户消息末尾的 `over` 改为 `[pending...]`（让用户看到正在处理），回复完成后删除 `[pending...]`
...
11. `##` 标题后有 `@delete` 标记时，删除该话题整段（从 `##` 到下一个 `##` 之前）
